...
{
	m_Mesh = Engine::Mesh(shader);
	m_Mesh.p_Usage = Engine::Mesh::Dynamic;
}
...
int Terrain::calculate_cube_index(std::vector<int> &cell, float isovalue)
{
	int cubeIndex = 0;
	for (int i = 0; i < 8; i++)
		if (m_ScalarField[cell[i]].scalar < isovalue)
			cubeIndex |= (1 << i);
	return cubeIndex;
}
														...
														float isovalue)
{
	...
	while (intersectionsKey) {
		if (intersectionsKey & 1) {
			...
			ScalarFieldPoint p1 = m_ScalarField[cell[v1]],
							 p2 = m_ScalarField[cell[v2]];

			glm::vec3 intersectionPoint = interpolate(
				p1.position, p1.scalar, p2.position, p2.scalar, isovalue);
...
glm::vec3 Terrain::interpolate(
	glm::vec3 &v1, float val1, glm::vec3 &v2, float val2, float isovalue)
{
...
void Terrain::MarchingCubes()
{
	...
	m_Mesh.p_Vertices = vertices;
	// for (auto vertex : vertices) std::cout << vertex.x << ", " << vertex.y <<
	// ", " << vertex.z   << std::endl;
	m_Mesh.p_Indices = indices;
	m_Mesh.UpdateMesh();
}
